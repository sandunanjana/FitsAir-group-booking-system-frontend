...
import { deleteGroupRequest, listGroupRequests, updateGroupRequest, listUsersByRole, type GroupRequestDTO } from "@/api/endpoints";
            ...
            {loading ? (
                ...
                <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-8 text-center">
                    ...
                    <p className="mt-1 text-sm text-gray-500">
                        {searchTerm || statusFilter !== "ALL" ? "Try adjusting your search or filter criteria" : "Get started by creating a new group request"}
                    </p>
                </div>
            ) : (
                <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
...
function Th({ children, className = "", ...rest }: ThHTMLAttributes<HTMLTableCellElement> & { children?: ReactNode }) {
    return <th className={`px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider ${className}`} {...rest}>{children}</th>;
}
